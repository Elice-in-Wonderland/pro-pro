...
import scss from "./createPost.scss";
...
//스택 추가
  appendStack() {
    const stackList = ['Javascript', 'React.js', 'Node.js', 'Vue.js', 'C', 'C++', 'C#', 'Typescript.js', 'Kotlin', 'Django', 'Go', 'Swift', 'Java', 'Spring', 'Fluter', 'Etc'];
    ...
    stackList.forEach((v,i,a) => {
      ...
      const stackInput = this.createDom('input', {
        type: 'checkbox', name: "stacks", value: v
      });
...
//시작일 추가
  appendStartDate() {
    ...
    this.transferData(startDateYear, startDateMonth, startDateDate, document.forms[0].startDate);
  }
...
//종료일 추가
  appendEndDate() {
    ...
    this.transferData(endDateYear, endDateMonth, endDateDate, document.forms[0].endDate);
  }
...
//마감일 추가
  appendRegisterDeadline() {
    ...
    this.defaultDate(registerDeadlineYear, registerDeadlineMonth, registerDeadlineDate);
    this.transferData(registerDeadlineYear, registerDeadlineMonth, registerDeadlineDate, document.forms[0].registerDeadline);
  }
...
//년월에 따른 일 변경, 서버 전송을 위한 date폼 data변경
    ...
    const registerDeadlineSelects = Array.from(document.querySelectorAll('.RegisterDeadline select'));

//사작일
    startSelects.forEach(v => {
      v.addEventListener('change', (e) => {
        this.dateAppendRemove(startSelects[0], startSelects[1], startSelects[2]);
        this.transferData(startSelects[0], startSelects[1], startSelects[2], e.target.parentElement.lastElementChild);
      })
    })
//종료일
    endSelects.forEach(v => {
      v.addEventListener('change', (e) => {
        ...
        this.transferData(endSelects[0], endSelects[1], endSelects[2], e.target.parentElement.lastElementChild);
      })
    })
//마감일
    registerDeadlineSelects.forEach(v => {
      v.addEventListener('change', (e) => {
        this.dateAppendRemove(registerDeadlineSelects[0], registerDeadlineSelects[1], registerDeadlineSelects[2]);
        this.transferData(registerDeadlineSelects[0], registerDeadlineSelects[1], registerDeadlineSelects[2], e.target.parentElement.lastElementChild);
      })
    })
  }
}